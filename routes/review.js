...
router.get('/', [auth, [
    ...
    check('lugger', 'lugger is required').not().isEmpty(),
]], async (req, res) => {
    ...
    try {
        ...
        let review = await Review.findOne({ lugger:req.body.lugger  })
        // console.log(review)
        if (!review) {
            return res
                .status(400)
...
async (req, res) => {
    ...
    try {
        ...
        const reviews = await  Review.find({lugger:req.body.lugger,user:req.user._id})
